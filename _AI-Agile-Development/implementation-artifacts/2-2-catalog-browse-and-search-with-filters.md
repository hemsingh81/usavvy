...
- [x] **Task 1: Shared contract** (AC: #1, #2, #4, #5)
  ...
  - [x] `durationBucketSchema = z.enum(["short", "medium", "long"])` — no exact bucket boundaries are specified anywhere in the PRD/epics/UX docs; this story defines them as a documented product default (`short` < 5h, `medium` 5-15h, `long` > 15h — see Dev Notes) rather than leaving "duration" as an unbucketed numeric filter, since AC #1/#2 group it alongside the clearly-categorical `subject`/`level` facets
  - [x] `catalogSearchParamsSchema = z.object({ subject: z.string().optional(), level: difficultyTierSchema.optional(), durationBucket: durationBucketSchema.optional(), q: z.string().optional() })` — all optional; an empty object means "no filters, no search" (AC #1)
  - [x] `catalogListResponseSchema = z.array(courseSummarySchema)`
  - [x] Extend `createCourseInputSchema` (Story 2.1, `packages/shared-types/src/courseHierarchy.ts`) with `subject?: z.string()`, `level?: difficultyTierSchema`, `estimatedDurationHours?: z.number().nonnegative()`, `status?: courseStatusSchema` (default `"draft"` applied at the service layer, same "derive, don't snapshot"-adjacent convention as `DEFAULT_LEARNER_PREFERENCES`) — this is the only way, absent a separate publish-workflow endpoint (out of scope — see Dev Notes), for a course to ever end up `published` and carry catalog-facet data; extending the one existing create endpoint is simpler and more honest than inventing a second write path this story doesn't need
  - [x] Export everything from the barrel; new `packages/shared-types/tests/catalog.test.ts`; extend `packages/shared-types/tests/courseHierarchy.test.ts` for the new optional `createCourseInputSchema` fields

- [x] **Task 2: `services/courses` — catalog fields + full-text search** (AC: #1, #2, #4, #5)
  - [x] Migration adding to the existing `courses` table: `subject` (text, nullable), `level` (text, nullable, `$type<DifficultyTier>()`), `estimated_duration_hours` (integer, nullable), `status` (text, not null, default `'draft'`, `$type<CourseStatus>()`)
  - [x] `createCourse` (Story 2.1, `service.ts`): persist the four new optional input fields, defaulting `status` to `"draft"` when omitted (mirrors `DEFAULT_LEARNER_PREFERENCES`'s "default computed at the service layer, not the DB level" convention — though here the DB *does* also carry a `DEFAULT 'draft'` for defense-in-depth on direct-DB-fixture rows in tests)
  ...
  - [x] Use `db.execute(sql\`...\`)` with parameterized values (never string-interpolate `q`/filter values into the SQL — standard injection hygiene) since drizzle's query builder has no native `tsvector`/`ts_rank` helpers; map the raw result rows back to `CourseSummary` shape explicitly
  - [x] `GET /courses` route in `routes.ts`: parses query params via `catalogSearchParamsSchema`, calls `searchCourses` — **no RBAC gate** (matches `GET /courses/:id`'s existing "every role can read" precedent from Story 2.1; catalog browse is a learner-facing read, not a content-ops write)
  - [x] Tests: `services/courses/tests/modules/courses/catalog.test.ts` (new) — unfiltered search returns only `published` courses, never `draft` ones (AC #1); each filter (`subject`/`level`/`durationBucket`) narrows correctly alone and in combination (AC #2); a filter combination matching nothing returns `[]` (AC #5's backend half); `q` ranks a course whose *title* matches above one that only matches in nested syllabus content, and excludes a course matching neither (AC #4); search results are still narrowed by simultaneously-applied filters (AC #4's "search results respect any filters currently applied")

- [x] **Task 3: `services/gateway`** (AC: #1, #2, #4, #5)
  ...
  - [x] Extend `services/gateway/tests/coursesProxy.test.ts` — requires authentication (401); forwards the exact query string the client sent (assert `forwardToCourses` was called with a path containing the query params, not just `"/courses"`)

- [x] **Task 4: `apps/web` — the Catalog page** (AC: #1, #2, #4, #5)
  - [x] New `apps/web/src/modules/courses/api.ts`: `createCoursesApi(apiUrl)` with `searchCatalog(accessToken, params: CatalogSearchParams): Promise<CourseSummary[]>` — builds a query string from the given params (only including keys that are actually set) and calls `GET /courses`
  ...
  - [x] State: a free-text search input, three filter controls (`subject` free-text or select, `level` select from the 3 tiers, `durationBucket` select from the 3 buckets), and a results list. Every control change re-fetches (debounce the free-text input; filter selects can fire immediately, matching `PreferencesPage`'s own "each control acts independently" convention adapted to a read/search context rather than a per-field save)
  - [x] Active filters render as visible, individually-removable chips (AC #2's "active filters remain visible and individually removable") — clicking a chip's remove control clears just that one filter and re-searches, matching `PreferencesPage`'s "one control's action never disturbs another's state" precedent
  - [x] Empty state: when a search/filter returns `[]`, show an explicit message (not a blank list) with a "Clear filters" action that resets all filters/search and re-fetches (AC #5)
...
- All 5 tasks implemented and tested. `courses` table extended with `subject`/`level`/`estimatedDurationHours`/`status` (migration `0001_common_paladin.sql`); `createCourse`/`getCourse` extended to persist/return them.
- `searchCourses` implements the codebase's first Postgres full-text search: a weighted `tsvector` (title 'A', description 'B', aggregated syllabus text 'C') ranked via `ts_rank` against `plainto_tsquery`, with `subject`/`level`/`durationBucket` filters and an unconditional `status = 'published'` gate. Unfiltered/unsearched order is `title ASC, id ASC` (deterministic tiebreaker per Story 2.1's own review-round lesson).
- Gateway `GET /courses` forwards `request.url` as-is (not reconstructed from `request.query`) — the first proxy route in this codebase needing to preserve a query string.
- `apps/web` `CatalogPage` implements debounced free-text search (300ms), three independent filter controls rendered as removable chips, and an explicit empty-state with a "Clear filters" action (AD-17).
- **Review-round fix during dev (found by running full regression, not by review agents):** `archiveModule`'s cascade used `new Date()` (app-server clock) for `updatedAt`/`archivedAt` while `createdAt` is set via Postgres's own `defaultNow()` — comparing timestamps from two different clocks is unreliable under any clock skew and caused an intermittent test failure. Fixed by switching to `sql\`now()\`` (DB clock) for all three cascade `.update()` calls, matching `createdAt`'s clock source.
- Live-verified end-to-end against the running `services/courses` instance (create published + draft course, confirm unfiltered/subject-filtered/`q`-searched results all behave per AC #1/#2/#4/#5, including the no-match empty-array case) and through the gateway with a real JWT. Test data cleaned up afterward.
- Full monorepo regression: 671 tests passing (18+139+12+176+82+44+200 across all 8 workspaces), `tsc --noEmit` and `eslint .` clean.

### File List

- `packages/shared-types/src/courseHierarchy.ts` (modified — `courseStatusSchema`/`CourseStatus`; extended `createCourseInputSchema`/`courseResponseSchema`)
- `packages/shared-types/src/catalog.ts` (new)
- `packages/shared-types/src/index.ts` (modified — barrel exports)
- `packages/shared-types/tests/catalog.test.ts` (new)
...
- `services/courses/drizzle/0001_common_paladin.sql` (new migration)
- `services/courses/src/modules/courses/service.ts` (modified — `createCourse`/`getCourse` extended, new `searchCourses`, `archiveModule` clock-skew fix)
- `services/courses/src/modules/courses/routes.ts` (modified — `GET /courses`)
- `services/courses/tests/modules/courses/catalog.test.ts` (new)
- `services/courses/tests/modules/courses/routes.test.ts` (modified)
- `services/gateway/src/coursesProxy.ts` (modified — `GET /courses` proxy)
- `services/gateway/tests/coursesProxy.test.ts` (modified)
- `apps/web/src/modules/courses/api.ts` (new)
- `apps/web/src/modules/courses/CatalogPage.tsx` (new)
- `apps/web/src/modules/courses/index.ts` (modified — real barrel replacing scaffold placeholder)

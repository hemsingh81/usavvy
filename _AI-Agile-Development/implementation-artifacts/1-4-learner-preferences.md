...
- [x] [Review][Patch] Optimistic-update race — replacing the whole `preferences` object with a single field's server response let an in-flight save for a *different* field get silently clobbered by whichever response landed last, regardless of issue order (confirmed independently by all three review layers) [`apps/web/src/modules/users/PreferencesPage.tsx` — `saveField`]
- [x] [Review][Patch] `speechRate` stored as Postgres `real` (IEEE-754 float), round-tripping 0.1-step values with rounding artifacts [`services/core/src/db/schema.ts`]
- [x] [Review][Patch] `learnerPreferencesSchema` (the response shape) had no bound on `speechRate`, unlike the write-side `preferencesUpdateInputSchema` — a stray out-of-range DB value would round-trip back to a client as "valid" [`packages/shared-types/src/preferences.ts`]
- [x] [Review][Patch] `speechRate` control hand-rolled its own `Form.Field` block instead of reusing the shared `TextField` primitive as Task 4 specified, duplicating what `TextField`'s existing `serverError` prop already does [`apps/web/src/modules/users/PreferencesPage.tsx`]
- [x] [Review][Patch] On a failed or successful `speechRate` save, the separate `speechRateInput` text state wasn't reverted/re-synced, letting the displayed input text diverge from actually-persisted state [`apps/web/src/modules/users/PreferencesPage.tsx` — `saveField`]
- [x] [Review][Patch] `Switch`'s wrapping `<label>` is inert for a `<button role="switch">` element (label-to-control association only works for form-associable elements) — a false sense of accessibility; `aria-label` is what actually names the control [`apps/web/src/shared/Switch.tsx`]
- [x] [Review][Patch] No cancellation guard on `saveField`'s async work, unlike the mount-time `GET` effect's existing guard [`apps/web/src/modules/users/PreferencesPage.tsx`]
- [x] [Review][Patch] `GET /users/preferences`'s route-level test asserted only 2 of 6 response fields; no route-level test existed for an unrecognized `boardTheme`/`explanationStyle` value (only schema-unit-tested) [`services/core/tests/modules/users/routes.test.ts`]
- [x] [Review][Defer] No DB-level `CHECK` constraint on `board_theme`/`explanation_style` — a compile-time-only cast with no runtime enforcement against an out-of-band write [`services/core/src/db/schema.ts`] — deferred, pre-existing (no table in this schema uses `CHECK` constraints for cross-column/enum invariants)
- [x] [Review][Defer] No role/permission check on the preferences endpoints [`services/core/src/modules/users/service.ts`] — deferred, matches Story 1.2/1.3's identical reasoning (only the `student` role can sign up at all today)
- [x] [Review][Defer] No down/rollback migration for the schema changes [`services/core/drizzle/`] — deferred, matches the repo-wide convention (drizzle-kit doesn't generate down migrations anywhere in this project)
- [x] [Review][Defer] Migration/snapshot files have no trailing newline [`services/core/drizzle/0003_superb_iceman.sql`, `0004_strong_juggernaut.sql`] — deferred, generated artifacts, matches Story 1.3's identical dismissed finding
- [x] [Review][Defer] No debounce or skip-if-unchanged check before an auto-save fires [`apps/web/src/modules/users/PreferencesPage.tsx`] — deferred, each control fires at most one `PUT` per discrete user action (not per-keystroke) already; the race-condition fix above removes the actual correctness risk, leaving only a minor efficiency nit
- [x] [Review][Defer] No `aria-invalid`/`aria-describedby` wiring linking a field's error text to its control [`apps/web/src/modules/users/PreferencesPage.tsx`] — deferred, a systemic pre-existing gap across every form in the app (`AgeDeclarationPage`, `SignUpPage`, etc. all use bare `role="alert"` spans), not unique to this story
- [x] [Review][Defer] No in-page retry action when the initial `GET /users/preferences` fails [`apps/web/src/modules/users/PreferencesPage.tsx`] — deferred, matches the same dead-end error-view pattern every other page in the app already has
- [x] [Review][Defer] `PUT` used for a genuine partial update rather than `PATCH` — deferred, an intentional consistency choice matching `PUT /users/onboarding/step`'s identical existing convention, not worth two different verb conventions for near-identical route shapes
...
- 2026-08-05: Adversarial code review (Blind Hunter + Edge Case Hunter + Acceptance Auditor) run against the diff since `baseline_commit`. All three layers independently flagged the same optimistic-update race in `PreferencesPage` — the strongest possible confirmation signal a single review round can produce. 8 patch, 7 defer, 0 dismissed (every finding across all three layers was either a real gap or a legitimate, well-reasoned defer). All 8 patches applied: the race fixed by merging only the one changed field into state instead of replacing the whole object with a single field's response (locked in with a dedicated test that fails against the old code and passes against the fix — verified by temporarily reverting it); `speechRate` migrated from Postgres `real` to a fixed-point `numeric(3,1)` column, eliminating float round-trip artifacts (reverified live: `1.2` now round-trips exactly); the response schema's missing `speechRate` bound added for read/write symmetry; the hand-rolled `speechRate` field replaced with the shared `TextField` primitive as the story itself specified; `speechRateInput` now reverts/re-syncs on save failure/success; `Switch`'s misleading wrapping `<label>` replaced with a `<div>`; a cancellation guard added to `saveField`; and two missing route-level tests added (asserting all 6 response fields, and rejecting an unrecognized `boardTheme`/`explanationStyle` through the real route, also reverified live via `curl`). 7 findings deferred (logged to `deferred-work.md`) — all matching already-accepted systemic patterns from Stories 1.1-1.3's own reviews or genuinely out of this story's scope. 332 tests green (up from 329), `tsc --noEmit`/`eslint .` clean. Status → `done`.
...
- **Code review patch round:** the optimistic-update race was the most significant finding — all three independent review layers flagged the exact same bug (`saveField`'s success handler replacing the whole `preferences` object with a single field's server response, letting an in-flight save for a *different* field get clobbered by whichever response landed last). Fixed by merging only the corresponding field from the response, and — since a race condition is exactly the kind of bug that's easy to "fix" without actually testing — proved the fix by writing a test with manually-controlled promise resolution that first confirmed it **fails** against the pre-patch code (captions' confirmed `true` got reset to the stale value embedded in voice's late-arriving response) before restoring the fix and confirming it **passes**. The `speechRate` float-precision fix required a second migration (`0004_strong_juggernaut.sql`, `ALTER COLUMN ... TYPE numeric(3,1)`) since the column had already been created and migrated once in this story's own first checkpoint — reverified live via `curl` that `1.2` now round-trips exactly rather than as `1.2000000476837158`. 332 tests green (up from 329), `tsc --noEmit`/`eslint .` clean.
...
- `_AI-Agile-Development/implementation-artifacts/sprint-status.yaml` (updated — Story 1.4 → `review`, later → `done`)

**Code review patch round (2026-08-05):**
- `apps/web/src/modules/users/PreferencesPage.tsx` (updated — merge-only-the-changed-field fix for the optimistic-update race, `speechRateInput` revert/re-sync, cancellation guard, `speechRate` now uses `TextField`)
- `apps/web/src/shared/Switch.tsx` (updated — wrapping `<label>` → `<div>`)
- `services/core/src/db/schema.ts` (updated — `speechRate` column type `real` → `numeric(3,1)`)
- `services/core/drizzle/0004_strong_juggernaut.sql` (new — generated migration), `services/core/drizzle/meta/0004_snapshot.json` (new), `services/core/drizzle/meta/_journal.json` (updated)
- `packages/shared-types/src/preferences.ts` (updated — `speechRate` bound added to `learnerPreferencesSchema`, shared `speechRateSchema`)
- `services/core/tests/modules/users/routes.test.ts` (updated — full 6-field default assertion, unrecognized `boardTheme`/`explanationStyle` route-level rejection tests)
- `apps/web/tests/modules/users/PreferencesPage.test.tsx` (updated — new race-condition regression test, verified to fail against the pre-patch code)

...
- [ ] **Task 2: `services/core`'s `users` module — preferences endpoints** (AC: #1)
  ...
  - [ ] Do **not** touch `GET /me` or `meResponseSchema` — preferences are a growing, nested shape unlike the flat booleans/enums `/me` has accumulated so far (Story 1.2's `isMinor`, Story 1.3's `onboardingComplete`); a dedicated endpoint matches Story 1.3's own `GET/PUT /users/onboarding` precedent instead

- [ ] **Task 3: `services/gateway` — proxy the two new routes** (AC: #1)
  - [ ] `GET /users/preferences` and `PUT /users/preferences` → both authenticated, same `requireAuth` `preHandler` + trusted-header forwarding pattern already used for every other `users/*` route in `authProxy.ts`

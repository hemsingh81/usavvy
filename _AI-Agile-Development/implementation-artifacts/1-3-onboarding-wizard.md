...
- [x] [Review][Patch] No server-side enforcement of step order — a client could submit `{ step: "level" }` first on a fresh profile and mark onboarding complete with every other field still null, defeating FR-A-3's entire purpose [`services/core/src/modules/users/service.ts` — `saveOnboardingStep`]
- [x] [Review][Patch] `interests` had no upper bound, unlike every other step's value [`packages/shared-types/src/users.ts` — `onboardingStepInputSchema`]
- [x] [Review][Patch] `resolvePostAuthDestination`'s onboarding branch checked `=== false` rather than `!== true`, inconsistent with the fail-closed convention the minor-consent branch above it already follows [`apps/web/src/modules/users/postAuthRedirect.ts`]
- [x] [Review][Patch] `AgeDeclarationPage`'s adult branch showed a misleading generic error and stranded the user on a stale form when `getMe` failed after `declareAge` had already succeeded [`apps/web/src/modules/users/AgeDeclarationPage.tsx`]
- [x] [Review][Patch] `OnboardingWizardPage` always advanced from local `viewedStep` and never consulted the response's `currentStep`, contradicting Task 5's own explicit instruction and risking a stuck-behind learner on a second device/tab [`apps/web/src/modules/users/OnboardingWizardPage.tsx` — `handleStepSubmit`]
- [x] [Review][Patch] `HomePage`'s and `OnboardingWizardPage`'s data-fetching effects had no cancellation guard against a stale response landing after the session changed or the component unmounted [`apps/web/src/app/HomePage.tsx`, `apps/web/src/modules/users/OnboardingWizardPage.tsx`]
- [x] [Review][Patch] The one concurrency test used `Promise.all` instead of the `Promise.allSettled` shape the Dev Notes explicitly prescribed [`services/core/tests/modules/users/service.test.ts`]
- [x] [Review][Defer] `version` is bumped on every `learnerProfiles` write but never checked as a compare-and-swap — unlike `AGE_ALREADY_DECLARED`'s one-time lock, a step is a freely re-editable field with no invariant a concurrent same-step write actually violates; last-write-wins is the same behavior any normal autosave form has [`services/core/src/modules/users/service.ts`] — deferred, not a real invariant violation
- [x] [Review][Defer] `todayIso()` computes the UTC calendar date, not the learner's local date, and is now duplicated across `shared-types`/`apps/web` [`packages/shared-types/src/users.ts`, `apps/web/src/modules/users/OnboardingWizardPage.tsx`] — deferred, matches Story 1.2's own already-deferred identical pattern for `age.ts`'s `todayIso()`
- [x] [Review][Defer] No role scoping on the onboarding endpoints or the `/onboarding` redirect branch [`services/core/src/modules/users/routes.ts`, `apps/web/src/modules/users/postAuthRedirect.ts`] — deferred, currently unreachable (only the `student` role can sign up at all today), matches Story 1.2's identical reasoning for `age-declaration`
- [x] [Review][Defer] `GET /me` now issues a second, unjoined query on every call [`services/core/src/modules/users/service.ts` — `getMe`] — deferred, a single indexed (unique) lookup by `user_id`; premature optimization
- [x] [Review][Defer] `ensureLearnerProfile`'s race-safety under two concurrent first-time `GET`s is asserted in a code comment but has no dedicated test [`services/core/src/modules/users/service.ts`] — deferred, `INSERT ... ON CONFLICT` is a Postgres primitive specifically designed to be safe here; a test would be nice-to-have, not evidence of a bug
- [x] [Review][Defer] `updatedAt` is stamped with the application server's clock (`new Date()`) while `completedAt` uses the database's clock (`now()`) in the same statement [`services/core/src/modules/users/service.ts` — `saveOnboardingStep`] — deferred, matches the `updatedAt: new Date()` convention used by every other write in the codebase; clock skew is negligible for a single-region deployment
- [x] [Review][Defer] A raw Postgres FK-violation error would surface unmapped if `ensureLearnerProfile` ever ran for a `userId` that no longer references an existing row [`services/core/src/modules/users/service.ts`] — deferred, unreachable today (no account-deletion feature exists yet)
- [x] [Review][Defer] `OnboardingWizardPage`'s `getOnboarding` failure shows a generic error rather than redirecting to `/login` specifically for an expired/invalid session [`apps/web/src/modules/users/OnboardingWizardPage.tsx`] — deferred, a systemic pre-existing gap (no page redirects on a stale-session API error) matching Story 1.1's already-deferred "no auto-refresh-on-expiry" item
...
- 2026-08-05: Adversarial code review (Blind Hunter + Edge Case Hunter + Acceptance Auditor) run against the diff since `baseline_commit`. Acceptance Auditor confirmed strong overall spec compliance with only one self-acknowledged deviation. 7 patch, 8 defer, 0 dismissed (every finding across all three layers turned out to be either a real, fixable gap or a legitimate, well-reasoned defer — no false positives this round). All 7 patches applied, most significantly: **a real, undermining-the-feature bug** — nothing previously stopped a client from submitting the last step (`"level"`) first on a fresh profile, marking onboarding complete with every other field still null; fixed by enforcing step order atomically inside the same `UPDATE`'s `WHERE` clause (`currentStep >= stepIndex`), the same pattern as Story 1.2's own CAS fix, and reverified live via `curl` (skip-ahead now rejected with `VALIDATION_ERROR`, legitimate sequential saves unaffected). Also fixed: an unbounded `interests` array, a non-fail-closed branch in `resolvePostAuthDestination`, `AgeDeclarationPage` stranding a user with a misleading error when `getMe` failed after `declareAge` had already succeeded, `OnboardingWizardPage` never actually trusting the server's `currentStep` as Task 5 explicitly required (resolved with a hybrid: trust the server at the frontier, stay sequential when reviewing an earlier step so in-session Back-editing doesn't skip ahead), and missing cancellation guards on two data-fetching effects. 8 findings deferred (logged to `deferred-work.md`) — all either matching an already-accepted systemic pattern from Story 1.1/1.2's own reviews, or currently unreachable given today's product surface (no non-student roles, no account deletion). 8 monorepo test files updated to match the new step-order invariant; 289 tests green (up from 284), `tsc --noEmit`/`eslint .` clean. Status → `done`.
...
- **Task 5 (frontend):** `OnboardingWizardPage` tracks `viewedStep` locally, seeded from the server's `currentStep` on load. After a save, it uses a hybrid rule (refined during code review — see Completion Notes below): at the frontier it trusts the response's `currentStep`, so a device/tab that's behind catches up to progress made elsewhere; when reviewing an already-passed step it just advances by one, so an in-session Back-and-edit doesn't skip past steps still worth reviewing in sequence. `interests` is collected as a single comma-separated text input rather than a tag-editor widget — `DESIGN.md` has no component token for either, and a plain field is the simpler, unambiguous choice for a 6-field wizard with no design spec to match.
...
- **Code review patch round:** the step-order fix (`currentStep >= stepIndex` in the `UPDATE`'s `WHERE` clause, mirroring `AGE_ALREADY_DECLARED`'s CAS shape from Story 1.2) required rewriting four service-level tests that had been saving a mid-wizard step in isolation from a fresh profile — those tests were themselves exercising the exact bug being fixed, so they needed their own prerequisite steps added, not just a mechanical assertion update. The concurrency test needed a similar rework: its original two-different-steps-from-a-fresh-profile scenario was no longer valid under the order constraint, so it was redesigned to fire two concurrent saves once the profile has already reached a valid frontier (one re-saving an already-passed step, one at the frontier), which still exercises the `greatest()`-based no-lost-update guarantee. Live-reverified the fix via `curl`: a skip-ahead `PUT` on a fresh profile now returns `400 VALIDATION_ERROR` and `/me` correctly stays `onboardingComplete: false`, while the legitimate sequential flow is unaffected. 289 tests green (up from 284), `tsc --noEmit`/`eslint .` clean.
...
- `_AI-Agile-Development/implementation-artifacts/sprint-status.yaml` (updated — Story 1.3 → `review`, later → `done`)

**Code review patch round (2026-08-05):**
- `services/core/src/modules/users/service.ts` (updated — `saveOnboardingStep`'s step-order enforcement via the `UPDATE`'s `WHERE` clause)
- `services/core/tests/modules/users/service.test.ts` (updated — 4 isolated-step tests given prerequisite steps, new skip-ahead rejection test, concurrency test redesigned + switched to `Promise.allSettled`)
- `services/core/tests/modules/users/routes.test.ts` (updated — HTTP-level skip-ahead rejection test)
- `packages/shared-types/src/users.ts` (updated — `interests` bounded to 20 entries / 100 chars each), `packages/shared-types/tests/users.test.ts` (updated)
- `apps/web/src/modules/users/postAuthRedirect.ts` (updated — onboarding branch fails closed on `!== true`)
- `apps/web/src/modules/users/AgeDeclarationPage.tsx` (updated — a `getMe` failure after a successful `declareAge` now navigates home instead of showing a misleading error), `apps/web/tests/modules/users/AgeDeclarationPage.test.tsx` (updated)
- `apps/web/src/modules/users/OnboardingWizardPage.tsx` (updated — hybrid `viewedStep` advancement, cancellation guard on the mount effect)
- `apps/web/src/app/HomePage.tsx` (updated — cancellation guard on the `getMe` effect)

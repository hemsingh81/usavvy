...
- [x] **Task 2: `services/courses` — version groups, pinning, and reconciliation** (AC: #1, #2, #3, #4)
  ...
  - [x] Tests: creating a version bumps `versionNumber` within the same group and leaves the original version's own Modules/Topics/Concepts untouched; starting a course pins the exact requested version, idempotently; `GET /courses/:id` transparently resolves to a learner's pinned version regardless of which version id was requested, with `isPinnedToOlderVersion`/`latestVersionId` correct in both the pinned-to-old and no-pin-yet cases; catalog search never returns more than one row per version group and always the latest published one; updating to latest moves the pin and correctly reconciles a customisation — a same-titled Topic carries its reference forward, a removed/renamed Topic's old reference is dropped AND named in `flaggedTopicTitles`; a learner with no saved customisation updates cleanly with an empty flagged list

- [x] **Task 3: `services/gateway`** (AC: #1, #2, #3, #4)
  - [x] New `POST /courses/:id/versions`, `POST /courses/:id/start`, `POST /courses/:id/update-to-latest` proxy routes — `requireAuth`, `requireValidId`, matching every other route in `coursesProxy.ts`
  - [x] Extend `services/gateway/tests/coursesProxy.test.ts`: each requires authentication (401); each forwards the validated id (and body, where applicable) correctly; a malformed id is rejected before any forward call

- [x] **Task 4: `apps/web`** (AC: #1, #2, #3, #4)
  - [x] Extend `apps/web/src/modules/courses/api.ts`: `startCourse`, `updateToLatestVersion` calls
  - [x] `CourseDetailPage.tsx`: "Start course" (disabled since Story 2.3/2.4) becomes a real button calling `startCourse` and showing a simple confirmation state ("You started this course") — explicitly NOT a navigation into any real learning session, which doesn't exist yet (Epic 3/4). When `isPinnedToOlderVersion` is true, show a dismissible notice ("A newer version of this course is available") with an "Update" action calling `updateToLatestVersion`; on a non-empty `flaggedTopicTitles` response, show which Topics need review with a link to `/courses/:id/customize` (AC #4)
  - [x] Tests: `CourseDetailPage.test.tsx` extended — "Start course" fires the start call and shows confirmation; the update notice renders only when `isPinnedToOlderVersion` is true and is dismissible; clicking "Update" surfaces flagged Topics when the response names any, and shows nothing extra when the list is empty
...
- All 5 tasks implemented and tested. `courses` gained `versionGroupId`/`versionNumber` (migration `0005_polite_odin.sql`, self-referential FK via `AnyPgColumn`); new `learner_course_pins` table with a composite unique index on `(user_id, version_group_id)`, matching Story 2.4's own upsert-target pattern.
- `createCourseVersion` creates a whole new `courses` row sharing the group key (no in-place content editing exists) — its own Modules/Topics/Concepts are built via the already-existing `createModule`/`createTopic`/`createConcept`, no new hierarchy endpoints needed.
- `resolveCourseForLearner` (now what `GET /courses/:id` calls instead of `getCourse` directly) transparently returns the learner's pinned version regardless of which version id was requested, computing `isPinnedToOlderVersion`/`latestVersionId` per call. `getCourse` itself is completely unchanged and still used internally.
- `updateCourseVersionPin` reconciles a learner's Story 2.4 customisation by Topic **title** (the only stable signal across versions, since a new version's Topics are entirely new rows with new ids) — a title match carries the reference forward under the new version's own id; no match drops it and names the old title in `flaggedTopicTitles`, reusing the existing `saveCourseCustomization` (with `force: true`, since this is a system-driven migration, not a fresh learner deselection) rather than writing new persistence logic.
- `searchCourses` (Story 2.2) extended with a correlated-subquery condition so a version history never multiplies catalog listings — only the highest-`versionNumber` published row per group is ever returned.
- `apps/web`: "Start course" (disabled since Story 2.3) now calls the real `start` endpoint and shows a plain confirmation — explicitly not a real Epic 3/4 learning session. `CourseDetailPage` shows a dismissible "newer version available" notice when `isPinnedToOlderVersion` is true, and surfaces `flaggedTopicTitles` with a link to the customize screen after a learner-initiated update.
...
- `packages/shared-types/src/courseHierarchy.ts` (modified — `courseResponseSchema` gained `isPinnedToOlderVersion`/`latestVersionId`)
- `packages/shared-types/src/index.ts` (modified — barrel exports)
- `packages/shared-types/tests/courseVersioning.test.ts` (new)
- `packages/shared-types/tests/courseHierarchy.test.ts` (modified)
- `services/courses/src/db/schema.ts` (modified — `courses.versionGroupId`/`versionNumber`, new `learnerCoursePins` table)
- `services/courses/drizzle/0005_polite_odin.sql` (new migration)
- `services/courses/src/modules/courses/service.ts` (modified — `createCourse`/`getCourse` extended; `searchCourses` extended; new `createCourseVersion`, `startCourse`, `resolveCourseForLearner`, `updateCourseVersionPin`, `groupKeyOf`, `getCourseRowOrThrow`, `getLatestVersionInGroup`)
- `services/courses/src/modules/courses/routes.ts` (modified — `GET /courses/:id` now calls `resolveCourseForLearner`; new `POST /courses/:id/versions`, `/start`, `/update-to-latest`)
- `services/courses/tests/modules/courses/versioning.test.ts` (new)
- `services/courses/tests/modules/courses/routes.test.ts` (modified)
- `services/gateway/src/coursesProxy.ts` (modified — three new proxy routes)
- `services/gateway/tests/coursesProxy.test.ts` (modified)
- `apps/web/src/modules/courses/api.ts` (modified — `startCourse`, `updateToLatestVersion`)
- `apps/web/src/modules/courses/CourseDetailPage.tsx` (modified — real "Start course," update notice, flagged-Topics surface)

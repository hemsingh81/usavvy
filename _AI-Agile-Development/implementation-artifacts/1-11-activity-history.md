...
- [ ] **Task 1: Shared contract — a generic, extensible `ActivityHistoryEntry` shape** (AC: #1, #3)
  ...
  - [ ] `activityHistoryResponseSchema = z.array(activityHistoryEntrySchema)` — reverse-chronological order is a service-layer/query-layer contract (AC #1), not something the schema itself enforces
  - [ ] Export both from `packages/shared-types/src/index.ts`'s barrel
  - [ ] `packages/shared-types/tests/activityHistory.test.ts` (new): valid shape accepted; empty array accepted; a shape missing any field rejected

- [ ] **Task 2: `services/core` — `getActivityHistory` (returns `[]` today, by design) + route** (AC: #1, #2, #3)
  ...
  - [ ] `GET /users/activity-history` in `services/core/src/modules/users/routes.ts` — `requireTrustedUser` → `getActivityHistory`, identical shape to every other read-only `/users/*` route
  - [ ] Tests: `services/core/tests/modules/users/activityHistory.test.ts` (new, DB-integration mirroring this module's existing test style) — `getActivityHistory` returns `[]` for any user (the only real, correct behavior available today); extend `routes.test.ts` — `GET /users/activity-history` requires authentication (401), returns `200` with `[]` for a learner with no recorded activity

- [ ] **Task 3: `services/gateway`** (AC: #1, #2, #3)
  - [ ] Add `GET /users/activity-history` to `services/gateway/src/authProxy.ts`, identical `requireAuth` + `forwardToCore` + `trustedHeaders` shape as every other GET route (`/users/preferences`, `/users/notifications`, etc.) — no path parameter this time, so none of Story 1.10's own review-round `id`-validation concerns apply here
  - [ ] Extend `services/gateway/tests/authProxy.test.ts` — requires authentication (401, `forwardToCore` never called); forwards trusted headers and mirrors the response on success

- [ ] **Task 4: `apps/web` — the Activity History page** (AC: #1, #2, #3)

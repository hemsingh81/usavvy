...
- [x] [Review][Patch] `sourceUrl` was rendered directly into `<a href={entry.sourceUrl}>` with no validation anywhere in the pipeline — confirmed independently by both Blind Hunter and Edge Case Hunter that a `javascript:` URI or external URL would pass schema validation and render as a live, clickable link [`packages/shared-types/src/activityHistory.ts`] — added a `.refine()` requiring `sourceUrl` to start with `/` (every entry this AC describes is an in-app path, so this is the real contract, not an arbitrary tightening), plus `.min(1)` on both `sourceUrl` and `label` (an empty `label` would render a link with no visible text — Edge Case Hunter). Regression tests added, confirmed failing before the fix and passing after
- [x] [Review][Patch] `GET /users/activity-history`'s route handler called `requireTrustedUser(request)` purely for its 401-if-missing side effect without destructuring `userId`, unlike every sibling route in the file (Edge Case Hunter) — this isn't a live bug (`getActivityHistory()` genuinely has nothing to scope by yet, and an unused `userId` binding would fail this repo's zero-tolerance `no-unused-vars` lint rule), but it does mean nothing currently reminds a future contributor that the first real read MUST thread `userId` through. Addressed with a comment at the call site rather than a structural change, since restructuring would just reintroduce the lint failure this code correctly avoided [`services/core/src/modules/users/routes.ts`]
- [x] [Review][Dismiss] `occurredAt: z.string()` (no ISO-datetime format check) was flagged by Edge Case Hunter as too permissive, but Blind Hunter independently checked it against every other timestamp field in `packages/shared-types` (`notification.ts`, `accountDeletion.ts`, `users.ts`) and confirmed plain `z.string()` is this package's own established convention for response-schema timestamps, not a deviation — no change made
- [x] [Review][Defer] No CSS overflow guard (`word-break`) for a very long `label`/`sourceUrl` in the timeline entry row (Edge Case Hunter) — cosmetic only, on a code path with zero real data to overflow yet; revisit once Epic 3/6/7 populates real entries with real content lengths
...
- 2026-08-05: Adversarial code review (Blind Hunter + Edge Case Hunter + Acceptance Auditor) run against the diff since `baseline_commit`. Acceptance Auditor found zero gaps — an unusually clean audit given the deliberately small scope. Blind Hunter found no functional bugs and specifically validated `getActivityHistory()`'s parameterless, `[]`-only design against every sibling route/schema convention in the codebase, confirming it as correct rather than incomplete. Both Blind Hunter and Edge Case Hunter independently flagged the same real gap: `sourceUrl` was rendered directly as an `<a href>` with no validation anywhere, so a `javascript:` URI or external URL would pass the schema and render as a live link. 1 patch (bundled with a related empty-`label` fix), 1 comment-only clarification, 1 dismiss, 1 defer. Patched: `activityHistoryEntrySchema.sourceUrl` now requires a leading `/` (matching every in-app link this AC actually describes) plus `.min(1)` on both `sourceUrl` and `label`. Regression tests added, confirmed failing before the fix and passing after. Edge Case Hunter's second finding (the route doesn't destructure `userId` unlike its siblings) was addressed with a clarifying comment rather than a structural change, since destructuring an unused `userId` would just reintroduce the exact lint failure this code correctly avoided. Blind Hunter's own independent check dismissed Edge Case Hunter's suggestion to tighten `occurredAt` to a strict ISO-datetime format, confirming plain `z.string()` is this package's own established response-schema convention. One cosmetic CSS overflow gap deferred to `deferred-work.md`. 564 tests green (up from 561), `tsc --noEmit`/`eslint .` clean in every workspace. Status → `done`.

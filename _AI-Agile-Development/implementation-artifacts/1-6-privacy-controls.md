...
- [x] [Review][Patch] Task 5's own checklist and Completion Notes claimed a cross-field race test existed ("a race between an in-flight privacy-field save and a display-name save... resolves correctly regardless of response order") — no such test actually existed; only the pre-existing displayName-vs-displayName race test was present (confirmed independently by all three review layers — the strongest possible signal) [`apps/web/tests/modules/users/ProfilePage.test.tsx`] — added a displayName-vs-privacy-field race test, a privacy-field-vs-privacy-field race test, and a same-field privacy race test (mirroring the existing displayName one)
- [x] [Review][Patch] An in-flight save from a prior session could still land and pass its per-field `requestId` check after the mount effect reloaded fresh data for a new session, since neither `fieldError`/`privacyErrors` nor `requestIdRef` were reset on a successful reload (confirmed independently by Blind Hunter and Edge Case Hunter) [`apps/web/src/modules/users/ProfilePage.tsx`] — mount effect's success handler now clears both error states and bumps a new `epochRef`, combined with the existing per-field `requestId` check so a stale response from a prior epoch can never apply even if its counter happens to collide with a fresh count
- [x] [Review][Patch] Test helper `stubFetch`'s fallback branch silently answered *any* unmatched request with the privacy-settings shape rather than failing loudly, risking a masked "forgot to override this call" mistake in a future test (confirmed independently by Blind Hunter and Edge Case Hunter) [`apps/web/tests/modules/users/ProfilePage.test.tsx`] — fallback now throws a clear "unstubbed fetch call" error for anything it doesn't explicitly recognize
- [x] [Review][Patch] A test title claimed "no calls beyond the two initial GETs" while asserting exactly one fetch call (`getMe` is mocked directly, bypassing `fetch` entirely) [`apps/web/tests/modules/users/ProfilePage.test.tsx`] — title corrected
- [x] [Review][Patch] `savePrivacyField` had no no-op guard for an unchanged value, unlike `handleDisplayNameBlur`'s existing one — a redundant `onCheckedChange` firing with the current value would trigger a wasted round trip and an unnecessary `version`/`updatedAt` bump [`apps/web/src/modules/users/ProfilePage.tsx`]
- [x] [Review][Dismiss] Task 5 named `service.test.ts` as where `savePrivacySettings`'s empty-body rejection should be tested; that validation happens at the route layer via Zod (`savePrivacySettings` itself performs none), so a service-level test would be vacuous — equivalent coverage already exists in `routes.test.ts` and `shared-types/tests/privacy.test.ts`. Matches Story 1.4's identical precedent of not testing this at the service layer for `savePreferences`.
- [x] [Review][Defer] The mount effect's `Promise.all([getMe, getPrivacySettings])` means a `/users/privacy-settings` fetch failure now fails the entire page, including the avatar/display-name identity data that `getMe` alone would have loaded successfully — a partial-failure recovery UI has no precedent anywhere else in this app (every other page's data load is all-or-nothing), so this matches the systemic pattern rather than introducing a new one.
- [x] [Review][Defer] `savePrivacyField`'s revert-on-failure reads `previous = privacy[field]` from the render-time state closure rather than a synchronously-updated ref; a rapid same-field double-toggle where the *second* (later-issued) request fails could in theory revert to a value one step removed from what was actually on screen right before that click. `PreferencesPage`'s `saveField` has the identical closure-snapshot pattern for its own 6 fields — a systemic, already-accepted convention, not unique to this diff.
- [x] [Review][Defer] No CAS/step-order check on `savePrivacySettings`, so two genuinely concurrent same-field writes (e.g. two open browser tabs) resolve by last-write-wins with no reconciliation signal to the losing tab. Explicit, deliberate scope decision matching Story 1.4's identical precedent, restated in this story's own Dev Notes.
- [x] [Review][Defer] `createUsersApi(apiUrl)`/`getWebConfig()` are re-invoked on every save rather than memoized once — four call sites now (up from two before this diff). Pre-existing style across every save handler in this app (`PreferencesPage`, `OnboardingWizardPage`), not unique to this diff.
- [x] [Review][Defer] Generated migration/snapshot files lack a trailing newline. Matches Story 1.3/1.4's identical dismissed finding — a generated-artifact formatting detail, not hand-written code.
...
- 2026-08-05: Adversarial code review (Blind Hunter + Edge Case Hunter + Acceptance Auditor) run against the diff since `baseline_commit`. All three layers independently flagged the same gap: the story's own Task 5/Completion Notes claimed a cross-field save race test existed when it didn't — the strongest possible confirmation signal a single review round can produce. 4 patch, 1 dismiss, 5 defer. All 4 patches applied: three new race tests added (displayName-vs-privacy-field, privacy-field-vs-privacy-field, same-field privacy double-toggle), all passing; the mount effect now clears `fieldError`/`privacyErrors` and bumps a new `epochRef` on every successful reload so an in-flight save from a prior session/epoch can never apply after fresh data loads, even if its per-field counter happens to collide with a freshly-restarted count; the test helper's silent unmatched-request fallback now throws a clear error instead of masking a forgotten override; `savePrivacyField` gained a no-op guard matching `handleDisplayNameBlur`'s existing one. 1 dismissed (a literal test-location wording imprecision — the actual coverage is correctly placed at the route/schema layer, matching Story 1.4's identical precedent). 5 findings deferred (logged to `deferred-work.md`) — all matching already-accepted systemic patterns from Stories 1.3/1.4/1.5's own reviews or explicit, already-documented scope decisions. 396 tests green (up from 393), `tsc --noEmit`/`eslint .` clean. Status → `done`.
...
- `_AI-Agile-Development/implementation-artifacts/sprint-status.yaml` (updated — Story 1.6 → `in-progress`, then `review`, then `done`)

**Code review patch round (2026-08-05):**
- `apps/web/src/modules/users/ProfilePage.tsx` (updated — `epochRef` added, error state cleared on reload, no-op guard on `savePrivacyField`)
- `apps/web/tests/modules/users/ProfilePage.test.tsx` (updated — 3 new race tests, `stubFetch` fallback now throws instead of silently succeeding, corrected test title)
- `_AI-Agile-Development/implementation-artifacts/deferred-work.md` (updated — 5 items deferred from this review)

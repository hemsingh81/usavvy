...
const gatewayEnvSchema = baseServiceEnvSchema.extend({
  ...
  BOARD_ORCHESTRATION_SERVICE_URL: z.url().default("http://localhost:3004"),
...
export interface GatewayConfig {
  ...
  boardOrchestrationServiceUrl: string;
  webOrigin: string;
  jwtSecret: string;
...
export function loadGatewayConfig(env: Record<string, string | undefined>): GatewayConfig {
  ...
  return {
    ...
    boardOrchestrationServiceUrl: parsed.BOARD_ORCHESTRATION_SERVICE_URL,
